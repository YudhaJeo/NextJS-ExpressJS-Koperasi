...
export default function AdjustPrintMarginLaporan({
  ...
  const addHeader = (doc, title, marginLeft, marginTop, marginRight) => {
    ...
    doc.text('Laporan Roles', pageWidth / 2, marginTop + 5, { align: 'center' });
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'Laporan Roles', marginLeft, marginTop, marginRight);
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Laporan Roles');
    XLSX.writeFile(wb, 'Laporan Roles.xlsx');
  };
