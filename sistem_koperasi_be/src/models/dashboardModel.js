...
export const getDashboardInfo = async () => {
  try {
    const totalUser = await db('users').count('* as total');
    const totalPerusahaan = await db('perusahaan').count('* as total');
    const totalSimpanan = await db('mutasitabungan').sum('Jumlah as total');
    const totalAktivasiMbank = await db('mbanking_username').sum('Jumlah as total');
    ...
    const simpananData = await db('simpanan')
      .select('Tgl', 'Faktur', 'Rekening', 'UserName', 'Nama', 'Alamat', 'DK', 'Nominal')
      .orderBy('Tgl', 'desc')
      .limit(50);
    ...
    const mutasiData = await db('mutasi')
      .select('Tgl', 'Faktur', 'Rekening', 'UserName', 'Nama', 'Alamat', 'Jumlah')
      .orderBy('Tgl', 'desc')
      .limit(50);

    const totalMutasi = await db('mutasi').sum('Jumlah as total');

    const chartData = {
      labels: ['User', 'Perusahaan', 'Simpanan', 'Mutasi'],
      datasets: [
        {
          label: 'Statistik',
